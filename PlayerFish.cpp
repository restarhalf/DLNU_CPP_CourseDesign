    ...
    bool PlayerFish::tryEat(AIFish& aiFish, ScoreManager& scoreManager)
    {
        ...
        if (SDL_HasIntersection(&rect, &other))
        {
            ...
            if (mySize > otherSize * 1.2f)
            {
                ...
                aiFish.kill();
                // 预计算增长后宽度
                float       wCurrent = sqrt(getSize());
                float       wNew = wCurrent * 1.04;
                if (wNew < wMax)
                    grow(1.02f);  // 体型增长 2%    别长太快
                else
                    resize();//判断并控制体积
                int value = aiFish.getScoreValue();             //特殊奖励
                int svalue = aiFish.getSpecialValue();
                if (svalue > 4)  // 0~5  六分之一的概率获得奖励分值
                {
                    value += value / 2;
